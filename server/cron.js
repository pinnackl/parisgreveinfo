...
axios.get(`${serviceAccount.apiUrl}/`, {responseType: 'json'})
  .then(response => {
    function isStrikeTomorrow(dates) {
      const date = new Date();
      if (dates[date.getMonth()] && dates[date.getMonth()].includes(date.getDate()+1)) return true;
      return false;
    }

    if (!isStrikeTomorrow(response.data)) return;
